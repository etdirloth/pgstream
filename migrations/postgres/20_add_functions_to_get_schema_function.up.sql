...
-- Update pgstream.get_schema function to include functions/procedures
...
functions AS (
    ...
    WHERE n.nspname = schema_name
      AND p.prokind IN ('f', 'p') -- focused on functions and procedures
),
columns AS (
    SELECT
...
fn_aggregated AS (
    SELECT jsonb_agg(jsonb_build_object(
        'oid', functions.function_oid,
        'name', functions.function_name,
        'definition', functions.function_definition,
...
as_json AS (
    SELECT
        jsonb_build_object(
            ...
            'functions',
            COALESCE((SELECT functions FROM fn_aggregated), '[]'::jsonb)
        ) AS v
    FROM by_table
)
...
-- Update pgstream.log_schema function to capture function events
...
BEGIN
    ...
    IF tg_event = 'sql_drop' THEN
        IF tg_tag = 'DROP SCHEMA' THEN
            ...
            END IF;
        ELSIF tg_tag IN ('DROP TABLE', 'DROP INDEX', 'DROP MATERIALIZED VIEW',
    ...
    ELSIF tg_event = 'ddl_command_end' THEN
        IF tg_tag = 'CREATE SCHEMA' THEN
            ...
            LIMIT 1;
        ELSIF tg_tag IN ('CREATE TABLE', 'ALTER TABLE', 'CREATE INDEX', 'CREATE UNIQUE INDEX', 'ALTER INDEX', 'SELECT INTO', 'CREATE TABLE AS', 'CREATE MATERIALIZED VIEW', 'ALTER MATERIALIZED VIEW',
